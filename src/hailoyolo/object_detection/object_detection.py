...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-n", "--net",
        type=str,
        default=str(Path(__file__).parent.parent / "assets" / "yolov8n.hef"),
        help="Path to the YOLO network in HEF format."
    )
    ...
    parser.add_argument(
        "-l", "--labels",
        type=str,
        default=str(Path(__file__).parent.parent / "assets" / "coco.txt"),
    ...
    parser.add_argument(
        ...
        help="Path to json config file for the object detection and tracking parameters."
    )
...
def run_inference_pipeline(net, input, config, batch_size, labels, output_dir,
          save_stream_output=False, resolution="sd",
          enable_tracking=False, show_fps=False, camera_index=0) -> None:
    """
    ...
    # Initialize input source from string: "picamera", "camera", video file, or image folder.
    cap, images = init_input_source(input, batch_size, resolution, camera_index)
    tracker = None
    fps_tracker = None
    if show_fps:
...
def main() -> None:
    ...
    """
    args = parse_args()
    run_inference_pipeline(args.net, args.input, args.config, args.batch_size, args.labels,
          args.output_dir, args.save_stream_output, args.resolution,
          args.track, args.show_fps, args.camera_index)
